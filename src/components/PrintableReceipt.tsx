      ...
      <View ref={ref} collapsable={false} style={[styles.paper, { width }]}>
        ...
        {items.map((item) => (
          <View key={item.id} style={styles.line}>
            ...
            <AppText style={styles.lineAmount}>{formatKyat(item.price * item.quantity)}</AppText>
          </View>
        ))}
        <View style={styles.dash} />
        <View style={styles.totalRow}>
          <AppText style={styles.totalLabel}>{t.receipt.total}</AppText>
          <AppText style={styles.totalValue}>{formatKyat(sale.total)}</AppText>
        </View>
        <View style={styles.dash} />
...
const styles = StyleSheet.create({
  ...
  shop: { textAlign: 'center', fontSize: 20, color: '#000', fontWeight: '700' },
  ...
  lineLeft: { flex: 1 },
  lineName: { fontSize: 14, color: '#000', fontWeight: '700' },
  lineMeta: { color: '#000', fontSize: 11, marginTop: 2 },
  lineAmount: { fontSize: 13, color: '#000', fontWeight: '700' },
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  totalLabel: { fontSize: 14, color: '#000' },
  totalValue: { fontSize: 18, color: '#000', fontWeight: '700' },
